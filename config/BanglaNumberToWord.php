...
class BanglaNumberToWord
{
    ...
    public function numToWord($number,$option=""){
        if (!is_numeric($number) ) return 'Not a Number';

        if(is_float($number)){
            $dot = explode(".", $number);
            return $this->numberSelector($dot[0]).' দশমিক '.$this->numToBnDecimal($dot[1]) ." ". $option;
        }else{
            return $this->numberSelector($number) ." ". $option;
        }
